...
from trainer import Trainer, AdverserialTrainer, NNTrainer, RLFTrainer,BoundaryUnlearning, TrainerSettings, count_parameters
...
def pruning_unleanring(model,seed,device):
...
def finetune_unlearning(args, model, dataloaders):
    ...
    trainer_settings = TrainerSettings(name = f"finetune_{args.SEED}_{model.name}",optimizer=optimizer_config, scheduler=scheduler_config, num_classes=args.data.num_classes,loss_fn=nn.CrossEntropyLoss(label_smoothing=0.1), log_path= args.directory.LOG_PATH,device=args.device, **{k:v for k,v in args.finetune.items() if k not in {"optimizer","scheduler","train","epochs"}} )
...
def scrubs_unlearning(args,model,dataloaders):
    optimizer_config = getattr(tr, args.finetune.optimizer.type + "OptimConfig" )(**args.finetune.optimizer)
    scheduler_config = getattr(tr, args.finetune.scheduler.type + "SchedulerConfig" )(**args.finetune.scheduler)
    trainer_settings = TrainerSettings(name = f"scrubs_{args.SEED}_{model.name}",optimizer=optimizer_config, scheduler=scheduler_config,num_classes=args.data.num_classes,loss_fn=nn.CrossEntropyLoss(label_smoothing=0.1), log_path= args.directory.LOG_PATH,device=args.device, **{k:v for k,v in args.finetune.items() if k not in {"optimizer","scheduler","train","epochs"}} )
...
def nearest_neighbor_unlearning(args, model, dataloaders):
    ...
    optimizer_config = getattr(tr, args.finetune.optimizer.type + "OptimConfig" )(** args.finetune.optimizer)
    scheduler_config = getattr(tr, args.finetune.scheduler.type + "SchedulerConfig" )(**args.finetune.scheduler)
    nn_trainer_settings = TrainerSettings(name = f"NN_{args.SEED}_{model.name}",optimizer=optimizer_config, scheduler=scheduler_config, num_classes=args.data.num_classes,log_path= args.directory.LOG_PATH,device=args.device,loss_fn=nn.CrossEntropyLoss(label_smoothing=0.1), **{k:v for k,v in args.finetune.items() if k not in {"optimizer","scheduler","train","epochs"}} )
...
def train(args,net,dataloaders):
    ...
    trainer_settings = TrainerSettings(name = f"train_{args.SEED}_{args.experiment}"  ,optimizer=optimizer_config, scheduler=scheduler_config,num_classes=args.data.num_classes, log_path= args.directory.LOG_PATH,device=args.device,loss_fn=nn.CrossEntropyLoss(label_smoothing=0.1), **{k:v for k,v in args.trainer.items() if k not in {"optimizer","scheduler","train","epochs"}} )
...
def main(args):
    ...
    train_loader,retain_loader, forget_loader, validation_loader,test_loader = get_cifar10_dataloaders(args,balanced=False) #get_celeba_dataloaders
    ...
    #torch.save(unleart_model.state_dict(), f"models/surgery_unlearning_{args.SEED}.pth")
    unlearnt_model = pruning_unleanring(net,args.SEED,args.device)
